...
def convert_examples_to_features(js,tokenizer,args,uid):
    #code
    ...
    return InputFeatures(code_tokens,code_ids,nl_tokens,nl_ids,js['url'],uid)
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None):
        ...
        for uid,js in enumerate(data):
            self.examples.append(convert_examples_to_features(js,tokenizer,args,uid))
...
def evaluate(args, model, tokenizer,file_name,eval_when_training=False):
    ...
    model.train()
#     # code_vecs=np.concatenate(code_vecs,0) # bad
#     # nl_vecs=np.concatenate(nl_vecs,0) # bad

#     scores=torch.matmul(nl_vecs,code_vecs.T) # bad

#     # sort_ids=np.argsort(scores, axis=-1, kind='quicksort', order=None)[:,::-1] # bad
#     sort_ids=torch.argsort(scores, dim=-1)[:,::-1]
    ...
    scores_t = torch.matmul(nl_vecs_t,code_vecs_t.T) # bad
    scores_t.to(torch.device('cuda:3'))
    del nl_vecs_t
    del code_vecs_t
# sort_ids=np.argsort(scores, axis=-1, kind='quicksort', order=None)[:,::-1] # bad
    sort_ids_t=torch.flip(torch.argsort(scores_t, axis=-1),dims=(1,))
    del scores_t
    nl_urls=[]
    code_urls=[]
